...
export class Logger {
  ...
  async action(
    ...
    user: User,
    domain: string,
    channelSentIn: GuildChannel | ThreadChannel | TextChannel | NewsChannel, //Changed this cus typing was bugging me
    taken: string[],
    failed: string[]
  ) {
    try {
  ...
  private async genLogMessage(
    ...
    if (taken.length) {
      ...
      if (guildConfig && guildConfig.logFormat && guildConfig.logFormat.length > 0) {
